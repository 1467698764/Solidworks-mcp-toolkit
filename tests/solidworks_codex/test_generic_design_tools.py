...
class GenericDesignToolsTests(unittest.TestCase):
    ...
    def test_workflow_plan_supports_single_part_and_part_to_assembly_loops(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(justification["execution_gate"]["tool"], "workflow-plan")
            self.assertEqual(justification["execution_gate"]["decision"], "block_full_rebuild_until_authorized")
            self.assertIn("backup_status", justification["execution_gate"]["required_evidence"])
            self.assertIn("selected_reason", justification["request_template"])
            self.assertEqual(justification["request_template"]["selected_reason"], "")
            self.assertEqual(justification["request_template"]["approval_status"], "pending")
