...
class ToolCatalogTests(unittest.TestCase):
    def test_catalog_extracts_mcp_tools_and_groups_workflows(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("solidworks_mate_intent_execute", names)
    ...
    def test_capability_matrix_maps_cli_mcp_safety_and_workflows(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(by_cli["mate-intent-execute"]["workflow"], "guarded_edit")
            self.assertEqual(by_cli["mate-intent-execute"]["safety"], "guarded_write")
            self.assertEqual(by_cli["mate-intent-execute"]["mcp"], "solidworks_mate_intent_execute")
            self.assertTrue(by_cli["mate-intent-execute"]["solidworks_required"])
            ...
            self.assertEqual(by_cli["standard-part-resolve"]["workflow"], "guarded_edit")
            self.assertEqual(by_cli["standard-part-resolve"]["safety"], "guarded_write")
            self.assertEqual(by_cli["standard-part-resolve"]["mcp"], "solidworks_standard_part_resolve")
            self.assertFalse(by_cli["standard-part-resolve"]["solidworks_required"])
            ...
            self.assertEqual(by_cli["visual-capture"]["workflow"], "verify_export")
            self.assertEqual(by_cli["visual-capture"]["safety"], "verification_or_export")
            self.assertEqual(by_cli["visual-capture"]["mcp"], "solidworks_visual_capture")
            self.assertFalse(by_cli["visual-capture"]["solidworks_required"])
            self.assertEqual(by_cli["visual-validate"]["mcp"], "solidworks_visual_validate")
            self.assertFalse(by_cli["visual-validate"]["solidworks_required"])
