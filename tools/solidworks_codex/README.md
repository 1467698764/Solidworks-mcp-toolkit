...
- `mcp/server.cjs` exposes the 56 MCP tools, including `solidworks_standard_part_resolve`, `solidworks_component_insert`, `solidworks_part_feature_execute`, `solidworks_metadata_execute`, `solidworks_part_geometry_validate`, `solidworks_mate_group_execute`, `solidworks_tool_catalog`, `solidworks_handoff_bundle`, and `solidworks_worklog`.
...
`part-feature-execute` supports reviewed extrude bosses, extrude cuts, revolve bosses, revolved cuts, basic holes, countersink holes, counterbore holes, slot cuts, pocket cuts, fillets, chamfers, linear/circular patterns, and mirrors. Boss/cut/revolve operations report operation role, reviewed sketch/axis selectors, feature name where supplied, depth or angle metadata, direction/through-all intent, and raw FeatureManager call evidence. Hole variants report operation role, reviewed center/depth/diameter metadata, and HoleWizard call evidence; each operation reports selection/call evidence from named feature/entity selectors.
...
`assembly-repair-plan` emits an `affected_subgraph` for each action. The subgraph lists local components, affected mates, native file paths, and the diagnosis evidence used to choose the boundary, keeping mate repair focused on the damaged area. Mutating repair actions also carry rollback preconditions that name the backup report, required backup-status result, and affected files before mate or component repair is executed.
