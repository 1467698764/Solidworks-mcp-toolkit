#!/usr/bin/env python3
"""Generate a display-grade SolidWorks bullhead shaper fixture.

This v2 generator is intentionally separate from the earlier lightweight validation
fixture. Its goal is recognizable machine-tool geometry: holes, slots, ribs, ways,
fasteners, table, vise, ram, crank disk, rocker arm, and tool head.
"""
...
def build_complete_shaper_spec() -> CompleteShaperSpec:
    out = "tools/solidworks_codex/live_fixture/shaper_machine_v4"
    parts = (
        ...
        PartSpec("column_frame_with_window", "frame", (155, 90, 330), "upright frame with large window and bearing bores", (F("frame_window_cut", 1, "arched service window"), F("bearing_bore", 2, "crank and rocker shaft bores"), F("rib", 4, "vertical web ribs"), F("hole", 8, "cover and bearing cap holes"), F("cut", 5, "window and bores"))),
        ...
        PartSpec("clapper_tool_head", "tool_head", (70, 60, 105), "clapper box and tool head with clamp holes", (F("bolt_hole_pattern", 4, "clapper cover bolts"), F("tool_slot_cut", 1, "vertical tool slot"), F("pin_bore", 1, "clapper pivot"), F("hole", 7, "bolts and pivot"), F("slot", 1, "tool slot"))),
        PartSpec("single_point_cutting_tool", "tool", (16, 12, 115), "visible cutting tool bit", (F("beveled_tip", 1, "angled cutting end"),)),
        ...
        PartSpec("slotted_rocker_arm", "rocker", (70, 22, 285), "slotted rocker arm with rounded ends and pivot holes", (F("long_slot_cut", 1, "sliding die slot"), F("pin_bore", 3, "pivot and link holes"), F("rounded_end", 2, "rounded fork ends"), F("hole", 3, "pivot/link holes"), F("slot", 1, "main slot"), F("cut", 4, "slot and bores"))),
        ...
        PartSpec("rocker_pivot_bracket", "bracket", (105, 56, 115), "forked bracket with pivot bore and bolt holes", (F("bearing_bore", 1, "rocker shaft bore"), F("bolt_hole_pattern", 4, "base bolts"), F("fork_slot_cut", 1, "fork relief"), F("hole", 5, "bore plus bolts"), F("slot", 1, "fork relief"))),
        ...
        PartSpec("table_cross_slide", "slide", (250, 150, 38), "cross slide under work table", (F("t_slot_cut", 2, "cross slide slots"), F("dovetail_cut", 1, "cross dovetail"), F("hole", 4, "feed screw holes"), F("slot", 2, "slide slots"))),
        PartSpec("work_table_with_t_slots", "table", (310, 175, 32), "work table with multiple T-slots", (F("t_slot_cut", 4, "work holding T-slots"), F("bolt_hole_pattern", 4, "table mounting holes"), F("hole", 4, "mounting holes"), F("slot", 4, "T-slots"))),
        PartSpec("vise_jaw_fixed", "vise", (95, 22, 45), "fixed vise jaw", (F("bolt_hole_pattern", 2, "jaw screws"), F("serrated_face", 1, "jaw serrations"), F("hole", 2, "jaw screw holes"))),
        PartSpec("vise_jaw_movable", "vise", (95, 22, 45), "movable vise jaw", (F("bolt_hole_pattern", 2, "jaw screws"), F("serrated_face", 1, "jaw serrations"), F("hole", 2, "jaw screw holes"))),
    ...
    return CompleteShaperSpec(
        ...
        output_dir=out,
        reports_dir="tools/solidworks_codex/reports/shaper_machine_v4",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default="tools/solidworks_codex/live_fixture/shaper_machine_v4")
    parser.add_argument("--reports-dir", default="tools/solidworks_codex/reports/shaper_machine_v4")
...
def placements_for(spec: CompleteShaperSpec) -> dict[str, tuple[float, float, float]]:
    return {
        "cast_bed_with_t_slots": (0, 0, 0), "column_frame_with_window": (-0.20, 0, 0.14),
        "left_dovetail_way": (0.03, -0.045, 0.27), "right_dovetail_way": (0.03, 0.045, 0.27),
        "ram_with_dovetail_and_tool_mount": (0.10, 0, 0.305), "front_gib_plate": (0.10, -0.065, 0.292),
        "rear_gib_plate": (0.10, 0.065, 0.292), "clapper_tool_head": (0.285, 0, 0.275),
        "single_point_cutting_tool": (0.325, 0, 0.215), "bull_gear_crank_disk": (-0.24, 0.085, 0.155),
        "crank_center_shaft": (-0.24, 0.085, 0.145), "eccentric_crank_pin": (-0.195, 0.085, 0.16),
        "bronze_sliding_die_block": (-0.15, 0.055, 0.205), "slotted_rocker_arm": (-0.13, 0.055, 0.225),
        "rocker_pivot_bracket": (-0.19, 0.055, 0.145), "rocker_pivot_shaft": (-0.19, 0.055, 0.19),
        "ram_drive_link": (0.03, 0.055, 0.235), "table_cross_slide": (0.08, 0, 0.07),
        "work_table_with_t_slots": (0.10, 0, 0.115), "vise_jaw_fixed": (0.08, -0.045, 0.15),
        "vise_jaw_movable": (0.16, -0.045, 0.15), "fastener_set_m6": (0.00, 0.11, 0.16),
        "washer_set": (-0.05, 0.11, 0.16), "oil_cups": (-0.08, -0.06, 0.33),
    }


def detail_instance_placements() -> dict[str, list[tuple[float, float, float]]]:
    bed_bolts = [(x, y, 0.035) for x in (-0.26, 0.0, 0.26) for y in (-0.095, 0.095)]
    table_bolts = [(x, y, 0.137) for x in (0.0, 0.20) for y in (-0.072, 0.072)]
    column_bolts = [(-0.255, y, 0.295) for y in (-0.038, -0.013, 0.013, 0.038)] + [(-0.145, y, 0.295) for y in (-0.038, -0.013, 0.013, 0.038)]
    link_washers = [(-0.24, 0.085, 0.171), (-0.195, 0.085, 0.197), (-0.19, 0.055, 0.205), (-0.15, 0.055, 0.224)]
    oil_cups = [(-0.11, -0.04, 0.345), (-0.11, 0.04, 0.345), (-0.24, 0.085, 0.178), (-0.19, 0.055, 0.215)]
    return {
        "fastener_set_m6": bed_bolts + table_bolts + column_bolts,
        "washer_set": bed_bolts[:4] + table_bolts + link_washers,
        "oil_cups": oil_cups,
    }
...
def construct_live_fixture(spec: CompleteShaperSpec, out_dir: Path, reports_dir: Path, force: bool) -> dict[str, Any]:
    ...
    result = {"ok": True, "assembly": str(asm_path.resolve()), "part_count": len(part_paths), "components": components, "skipped_locked_files": skipped}
