...
| Slots and pockets | present/guarded | `part-feature-execute` / `solidworks_part_feature_execute` creates reviewed straight slot cuts and rectangular pocket cuts from a selected plane/face with center, width/length/height, depth, sketch creation, cut call evidence, and slot/pocket operation roles. Rich slot-interface readback remains a validation-layer track. | Slot role and dimensions read back. |
