...
| Standard part attachment | present/guarded | Hostless standard parts are detected and grouped into candidate concentric/coincident attachment plans; `component-insert` / `solidworks_component_insert` places reviewed `.SLDPRT`/`.SLDASM` standard/detail components by origin/config/fixed-state spec while preserving standard-part role, attachment role, host component, host interface id, optional host/inserted selectors with native identity envelopes, mate group id, required mate types, selector handoff status, and awaiting-mate-group status for downstream execution. Hardware library selection policy remains its own sourcing concern. | No accepted standard/detail component is isolated or hostless, and inserted standard parts carry an attachment contract plus selector handoff before mate execution. |
