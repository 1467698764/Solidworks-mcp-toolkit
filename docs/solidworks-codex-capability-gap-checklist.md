...
| Width/symmetry | partial/guarded | `width` mate groups can carry four reviewed planar face selectors, generate a width-specific review macro, and execute through `CreateMateData(11)` / `CreateMate` with width and tab face sets. Symmetry mate execution remains separate. | Remaining DOF matches intent. |
