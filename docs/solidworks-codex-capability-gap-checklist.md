...
| Slot/path/cam/gear mates | partial/guarded | `gear` mate groups can carry two reviewed cylindrical/axis selectors plus positive numerator/denominator ratio, generate review macros, and execute through native selection plus `AddMate5`. Slot/path/cam remain separate mechanism-specific tracks. | Optional until standard mechanism_lite is stable. |
