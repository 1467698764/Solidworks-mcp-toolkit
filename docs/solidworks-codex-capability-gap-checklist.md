...
| Feature suppression/edit | present/guarded | `feature-state` / `solidworks_feature_state` executes suppress, unsuppress, delete, and feature-scoped dimension writes by exact or unique feature-name match with live rebuild, before/after state evidence, feature count delta, selection evidence, change scope, and operation roles for deactivation, reactivation, removal, or parameter adjustment. Reorder and richer definition-object edits remain separate capability tracks. | Rebuild health, feature count, feature state, operation role, change scope, and edited parameter values are read back. |
...
| Selective part repair | present/guarded | `feature-state` can edit an affected feature state or feature-scoped dimension while preserving before/after evidence, exact changed feature, changed parameter, parameter delta, feature count delta, operation role, and change scope; broader sketch/topology surgery remains a separate capability track. | Report lists changed feature, changed parameter, parameter delta, rebuild result, and downstream checks. |
