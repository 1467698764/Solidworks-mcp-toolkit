...
| Selective mate repair | present/guarded | Reviewable mate group macro drafts, selector-bearing execution manifests, `mate-group-execute`, and live protocols can be generated per actionable group; bad-mate suppress/delete actions propagate from repair plans into execution manifests and run before replacement mates, while repair actions expose affected subgraphs for backup scope and local mutation boundaries. Stronger native entity ids still need live hardening. | Untouched components keep file timestamps/transforms unless affected, and each changed group has rebuild/inspect/execution-check evidence. |
