...
| Feature suppression/edit | present/guarded | `feature-state` / `solidworks_feature_state` executes suppress, unsuppress, delete, feature-scoped dimension writes, and reviewed feature reorder before/after a target feature by exact or unique feature-name match with live rebuild, before/after state evidence, feature count delta, feature order delta, selection evidence, change scope, and operation roles for deactivation, reactivation, removal, parameter adjustment, or tree reorder. Richer definition-object edits remain a separate capability track. | Rebuild health, feature count, feature state/order, operation role, change scope, and edited parameter values are read back. |
...
| Selective part repair | present/guarded | `feature-state` can edit an affected feature state, feature-scoped dimension, or feature tree order while preserving before/after evidence, exact changed feature, changed parameter or reorder target, parameter delta, feature count delta, feature order delta, operation role, and change scope; broader sketch/topology surgery remains a separate capability track. | Report lists changed feature, changed parameter or reorder target, parameter/order delta, rebuild result, and downstream checks. |
