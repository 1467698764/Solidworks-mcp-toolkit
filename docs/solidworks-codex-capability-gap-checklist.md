...
| Slot/path/cam/gear mates | partial/guarded | `gear` mate groups carry two reviewed cylindrical/axis selectors plus positive numerator/denominator ratio; `cam` / `cam_follower` mate groups carry two reviewed face/edge selectors. Both generate review macros and execute through native selection plus `AddMate5`. Slot/path remain separate mechanism-specific tracks. | Optional until standard mechanism_lite is stable. |
