...
The MCP wrapper currently exposes **48 MCP tools** across read-only inspection, analysis, handoff, guarded writes, export/verify, release gates, and the optional live SolidWorks gate.
...
`workflow-plan` maps `-Target` to the overall CAD goal, `-Action` to intent (`auto`, `single_part`, `part_to_assembly`, `assembly`, or `mechanism_assembly`), and `-View` to runtime budget (`fast`, `standard`, `strict`; `auto` uses `standard`). When `-Action` is omitted, `swctl` passes `auto` so the target text is classified instead of defaulting to one-part work.
Its JSON output includes `intent_classification` with source, detected signals, CAD scope, resolved intent, selected profile, and non-goals before mutation is planned. It also includes `design_intent` with scope, validation profile, inferred parts, subassemblies, interfaces, motion pairs, standard parts, editable parameters, and non-goals. It also includes `validation_profile_selection` with the selected profile, source profile, blocking/warning/not-applicable checks, stage profile mapping, runtime budget, and acceptance rule. Small part work keeps assembly and motion checks not-applicable, while mechanism work selects `mechanism_lite` instead of accepting smoke-level evidence. The output also includes an `assumption_ledger` artifact that separates `assumption`, `warning`, and `blocker` items for dimensions, materials, simplified geometry, validation scope, write safety, assembly interfaces, and mechanism evidence. Treat blocker entries as stage gates until the required evidence or an explicit rescope is recorded. It also includes `runtime_budget_plan` with expected SolidWorks sessions, rebuild scope, memory ceiling, timeout, cleanup policy, extra-check policy, and a `full_rebuild_justification_policy` that keeps local repair as the default and blocks full rebuilds unless a supported reason and evidence fields are recorded.
