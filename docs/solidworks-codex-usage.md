...
`base_plate-1:plane:z_max`, normals, local frames, role, confidence,
`confidence_level`, `selection_policy`, source evidence, and fallback `selector`
records for reopen/repair. Weak bbox-only candidates block automatic selection
until live face/axis evidence confirms exact SolidWorks entities.
